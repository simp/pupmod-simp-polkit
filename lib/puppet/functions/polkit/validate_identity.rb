  ...
  def validate_identities(identities)
    ...
    identities.each do |entry|
      ...
      unless valid_headers.include?(header)
        raise("polkit::validate_identity(): Error, identity specifier '#{header}' must be one of '#{valid_headers.join(', ')}' for entry '#{entry}'")
      end

      valid_name = %r{^[A-Za-z0-9_.*-]+$}
      unless valid_name.match(val)
        raise("polkit::validate_identity(): Error, value '#{val}' is invalid for entry '#{entry}'")
      end

      if header == 'unix-netgroup' && val.include?('*')
        raise("polkit::validate_identity(): Error, value '#{val}' cannot contain glob for entry '#{entry}'")
      end
    end
  end
end
