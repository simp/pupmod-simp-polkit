  ...
  hosts.each do |host|
    ...
    context "Set hidepid but not gid on /proc on #{host}" do
      it 'remounts /proc with hidepid=2' do
        skip 'Reboot/remount of /proc is not supported under containers' if host[:hypervisor] == 'docker'
    ...
    context "Set hidepid and gid on /proc on #{host}" do
      ...
      it 'remounts /proc with hidepid=2 and gid=100' do
        skip 'Remount of /proc is not supported under containers' if host[:hypervisor] == 'docker'
